...
// ─── Trainingstypes uit het PDF-schema van hardloopschema.nl ───────────────
// Loopsnelheden (op basis van 7510m in 35:08):
//   H  (herstel)   6:41/km = 0.1496 km/min
//   D1 (makkelijk) 5:51/km = 0.1709 km/min
//   D2 (gemiddeld) 5:12/km = 0.1923 km/min
//   D3 (zwaar)     4:41/km = 0.2136 km/min
//   W  (interval)  4:27/km = 0.2247 km/min

type Sessie = {
  dag: number           // 0=ma … 6=zo
...
const PLAN: Sessie[][] = [
  ...
  [
    ...
    { dag: 3, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 4, type: 'hardlopen', intensiteit: 'makkelijk',  beschrijving: 'Duurloop 40 min in D1',                                                duur_minuten: 40,  afstand_km: 6.8 },
    { dag: 5, type: 'hardlopen', intensiteit: 'interval',   beschrijving: 'Interval: 5 × 1000m + 2 × 2000m, eindigen in D2-D3 (2-3 min wandelen) – alternatief: halve marathon', duur_minuten: 70, afstand_km: 13.0 },
  ...
  [
    ...
    { dag: 3, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 4, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 5, type: 'hardlopen', intensiteit: 'makkelijk',  beschrijving: 'Duurloop 90 min in D1',                                                duur_minuten: 90,  afstand_km: 15.4 },
    { dag: 6, type: 'hardlopen', intensiteit: 'makkelijk',  beschrijving: 'Fartlek: 45 min D1 met 3 × 2 min versnellen naar D2 – gebruik je parcours', duur_minuten: 45, afstand_km: 7.7 },
  ],
  // ── Week 14 (laatste week) ─────────────────────────────────────────────
  [
    { dag: 0, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 1, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 2, type: 'hardlopen', intensiteit: 'makkelijk',  beschrijving: 'Activering: 50 min D1 – laatste 5 min: 3 × 20 sec versnelling (80-90%)', duur_minuten: 50, afstand_km: 8.5 },
    ...
    { dag: 4, type: 'rust',      intensiteit: 'herstel',    beschrijving: 'Rust – geen training',                                                 duur_minuten: null, afstand_km: null },
    { dag: 5, type: 'hardlopen', intensiteit: 'herstel',    beschrijving: 'Activering: 30 min D1 met 3 × 20 sec versnelling – rek daarna je spieren', duur_minuten: 30, afstand_km: 5.1 },
    { dag: 6, type: 'hardlopen', intensiteit: 'zwaar',      beschrijving: '🏁 MARATHON – geef alles, je hebt 14 weken keihard getraind!',          duur_minuten: 195, afstand_km: 42.2 },
  ],
]
...
export async function POST() {
  try {
    ...
    // Haal actief doel op (voor marathon datum)
    ...
    // Week 14, Zondag = marathondatum
    // Week 1, Maandag = marathonDatum - 97 dagen
    // Formule: sessiedatum = marathonDatum - (14 - weekNr) * 7 - 6 + dagOffset
    function berekenDatum(weekNr: number, dag: number): string {
      const datum = new Date(marathonDatum)
      datum.setDate(datum.getDate() - (14 - weekNr) * 7 - 6 + dag)
    ...
    if (deleteError) {
      return NextResponse.json({ error: `Fout bij verwijderen: ${deleteError.message}` }, { status: 500 })
    }

    // Bouw alle sessies op
    const sessies = []
    let volgorde = 1

    for (let weekIdx = 0; weekIdx < PLAN.length; weekIdx++) {
      const weekNr = weekIdx + 1
      ...
      for (const s of week) {
        sessies.push({
          user_id: user.id,
          goal_id: doel.id,
          datum: berekenDatum(weekNr, s.dag),
          type: s.type,
    ...
    // Voeg in batches in (Supabase max 1000 per insert)
    const BATCH = 100
    for (let i = 0; i < sessies.length; i += BATCH) {
      const { error } = await supabase.from('training_sessions').insert(sessies.slice(i, i + BATCH))
      if (error) {
        return NextResponse.json({ error: `Fout bij invoegen (batch ${i}): ${error.message}` }, { status: 500 })
      }
    }
    ...
    return NextResponse.json({
      ...
      startDatum: berekenDatum(1, 0),
      eindDatum: doel.datum,
      bericht: `${sessies.length} sessies aangemaakt voor 14-weeks schema (${berekenDatum(1, 0)} t/m ${doel.datum})`,
    })
